...
xor_not_use_avoid_cnt_by_cat = {"warning": 0, "prohibitory": 0, "mandatory": 0}
for _c1_str, _c1, _c2s in avoid:
    _c2_cnt = len(constants.ALL_SIGNS_BY_CATEGORY[_c1_str])
    for _c2 in _c2s:
        _c2 = np.array(_c2)
        __cat_2_dec = utils.num_bin_2_dec(num=_c2)
        if __cat_2_dec <= _c2_cnt - 1:
            xor_not_use_avoid_dec_set.add(__cat_2_dec)
            xor_not_use_avoid_cnt_by_cat[_c1_str] += 1
        _c2_real = _c1 ^ _c2
        __cat_2_dec_real = utils.num_bin_2_dec(num=_c2_real)
