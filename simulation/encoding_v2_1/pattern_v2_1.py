...
_all_cat_2_bin = [[0, i1, 0, i3, 0, i5, 0, i7]
                 ...
                 [[i0, 0, i2, 0, i4, 0, i6, 0]
                  for i0 in range(2) for i2 in range(2) for i4 in range(2) for i6 in range(2)]  # on-board bin
...
for _cat_idx in range(constants.CNT_CATEGORY_1):  # todo: remove impossible to-be-avoided cat_2 idx
    for __idx, __avoid_idx in enumerate(_ALL_BIN_PATTERNS_AVOID_DEC_IDX):
        __sub = constants.CNT_CATEGORY_2 + __idx
        _ALL_BIN_PATTERNS_DEC_IDX_SUBSTITUTION_ORI_2_SUB[_cat_idx][__avoid_idx] = __sub
        _ALL_BIN_PATTERNS_DEC_IDX_SUBSTITUTION_SUB_2_ORI[_cat_idx][__sub] = __avoid_idx
...
def dec_2_non_dup_bin(num: int, digit: int, category_1_idx: int, is_category_2: bool = False) -> np.ndarray:
    """
...
def non_dup_bin_2_dec(num: np.ndarray, category_1_idx: int, is_category_2: bool = False) -> int:
    """
